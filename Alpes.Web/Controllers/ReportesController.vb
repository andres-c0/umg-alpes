    ...
    Public Class ReportesController
        ...
        Private Function ExportarExcel(periodo As String,
            ...
            Using wb As New XLWorkbook()
                ...
                ws.Cell(1, 1).Value = "MUEBLES DE LOS ALPES"
                ws.Cell(2, 1).Value = "Reporte administrativo ejecutivo"
                ws.Cell(3, 1).Value = "Periodo"
                ws.Cell(3, 2).Value = periodo
                ws.Cell(4, 1).Value = "Año"
                ws.Cell(4, 2).Value = anio

                ws.Cell(6, 1).Value = "Indicador"
                ws.Cell(6, 2).Value = "Valor"
                ws.Cell(7, 1).Value = "Ventas totales"
                ws.Cell(7, 2).Value = data.VentasTotales
                ws.Cell(8, 1).Value = "Órdenes"
                ws.Cell(8, 2).Value = data.Ordenes
                ws.Cell(9, 1).Value = "Clientes únicos"
                ws.Cell(9, 2).Value = data.Clientes
                ws.Cell(10, 1).Value = "Ticket promedio"
                ws.Cell(10, 2).Value = data.TicketPromedio
                ws.Cell(11, 1).Value = "Items vendidos"
                ws.Cell(11, 2).Value = data.ItemsVendidos
                ws.Cell(12, 1).Value = "Canceladas"
                ws.Cell(12, 2).Value = data.Canceladas
                ws.Cell(13, 1).Value = "Valor por cliente"
                ws.Cell(13, 2).Value = data.ValorPorCliente
                ws.Cell(14, 1).Value = "Recompra"
                ws.Cell(14, 2).Value = data.PorcentajeRecompra / 100D

                EstilizarHoja(ws, "A6:B14", 6)
                ws.Range("B7:B7").Style.NumberFormat.Format = "Q #,##0.00"
                ws.Range("B10:B10").Style.NumberFormat.Format = "Q #,##0.00"
                ws.Range("B13:B13").Style.NumberFormat.Format = "Q #,##0.00"
                ws.Range("B14:B14").Style.NumberFormat.Format = "0.0%"

                ' ===================== VENTAS MENSUALES =====================
                Dim wsMeses = wb.Worksheets.Add("Ventas Mensuales")
                wsMeses.Cell(1, 1).Value = "Mes"
                wsMeses.Cell(1, 2).Value = "Ventas"
                wsMeses.Cell(1, 3).Value = "Órdenes"
                ...
                For Each item In data.VentasMensuales
                    ...
                    fila += 1
                Next
                EstilizarHoja(wsMeses, "A1:C" & Math.Max(fila - 1, 1).ToString(), 1)
                wsMeses.Column(2).Style.NumberFormat.Format = "Q #,##0.00"

                ' ===================== ÚLTIMAS ÓRDENES =====================
                Dim wsOrdenes = wb.Worksheets.Add("Ordenes")
                wsOrdenes.Cell(1, 1).Value = "Orden"
                wsOrdenes.Cell(1, 2).Value = "Fecha"
                wsOrdenes.Cell(1, 3).Value = "Cliente"
                wsOrdenes.Cell(1, 4).Value = "Estado"
                wsOrdenes.Cell(1, 5).Value = "Total"
                ...
                For Each o In data.UltimasOrdenes
                    ...
                    fila += 1
                Next
                EstilizarHoja(wsOrdenes, "A1:E" & Math.Max(fila - 1, 1).ToString(), 1)
                ...
                wsProductos.Cell(1, 1).Value = "#"
                wsProductos.Cell(1, 2).Value = "Producto"
                wsProductos.Cell(1, 3).Value = "Tipo"
                wsProductos.Cell(1, 4).Value = "Cantidad"
                wsProductos.Cell(1, 5).Value = "Ventas"
                wsProductos.Cell(1, 6).Value = "% total"
                ...
                For Each p In data.ProductosTop
                    ...
                    pos += 1
                Next
                EstilizarHoja(wsProductos, "A1:F" & Math.Max(fila - 1, 1).ToString(), 1)
                ...
                wsCompras.Cell(1, 1).Value = "Fecha compra"
                wsCompras.Cell(1, 2).Value = "Cliente"
                wsCompras.Cell(1, 3).Value = "Valor"
                wsCompras.Cell(1, 4).Value = "Forma pago"
                wsCompras.Cell(1, 5).Value = "Muebles incluidos"
                ...
                For Each c In data.ComprasPorCliente
                    ...
                    fila += 1
                Next
                EstilizarHoja(wsCompras, "A1:E" & Math.Max(fila - 1, 1).ToString(), 1)
                ...
                wsIndicadores.Cell(1, 1).Value = "Mes"
                wsIndicadores.Cell(1, 2).Value = "Ventas"
                wsIndicadores.Cell(1, 3).Value = "Órdenes"
                wsIndicadores.Cell(1, 4).Value = "Clientes activos"
                wsIndicadores.Cell(1, 5).Value = "Ticket promedio"
                ...
                For Each m In data.IndicadoresMensuales
                    ...
                    fila += 1
                Next
                EstilizarHoja(wsIndicadores, "A1:E" & Math.Max(fila - 1, 1).ToString(), 1)
                ...
                wsDetalle.Cell(1, 1).Value = "Mes"
                wsDetalle.Cell(1, 2).Value = "Producto"
                wsDetalle.Cell(1, 3).Value = "Cantidad"
                wsDetalle.Cell(1, 4).Value = "Ventas"
                wsDetalle.Cell(1, 5).Value = "% total"
                ...
                For Each d In data.DetalleMensualProductos
                    ...
                    fila += 1
                Next
                EstilizarHoja(wsDetalle, "A1:E" & Math.Max(fila - 1, 1).ToString(), 1)
                ...
                wsCohorte.Cell(1, 1).Value = "Cohorte"
                wsCohorte.Cell(1, 2).Value = "Clientes"
                wsCohorte.Cell(1, 3).Value = "M0"
                wsCohorte.Cell(1, 4).Value = "M+1"
                wsCohorte.Cell(1, 5).Value = "M+2"
                wsCohorte.Cell(1, 6).Value = "M+3"
                wsCohorte.Cell(1, 7).Value = "Ventas"
                ...
                For Each c In data.Cohortes
                    ...
                    fila += 1
                Next
                EstilizarHoja(wsCohorte, "A1:G" & Math.Max(fila - 1, 1).ToString(), 1)
        ...
        Private Sub EstilizarHoja(ws As IXLWorksheet, rango As String, headerRow As Integer)
            ...
            Dim header = ws.Row(headerRow)
            header.Style.Font.Bold = True
            header.Style.Font.FontColor = XLColor.White
            header.Style.Fill.BackgroundColor = XLColor.FromHtml("#2C1810")
            ...
            If ws.Name = "Resumen" Then
                ...
                ws.Cell("A1").Style.Fill.BackgroundColor = XLColor.FromHtml("#2C1810")

                ws.Range("A2:B2").Merge()
                ws.Cell("A2").Style.Font.Italic = True
                ws.Cell("A2").Style.Fill.BackgroundColor = XLColor.FromHtml("#F7F3EE")
            End If

            If ws.RangeUsed().RowCount() > headerRow Then
