...
Namespace Controllers
    Public Class PortalClienteController
        ...
        <HttpPost>
        Function FusionarCarritoInvitadoData() As ActionResult
            Dim cliId As Integer = 0
            ...
            Try
                Dim body As JObject = LeerBodyComoJObject()
                Dim itemsToken As JToken = body("items")

                If itemsToken Is Nothing Then
                    itemsToken = body("Items")
                End If

                Dim cantidadesPorProducto As New Dictionary(Of Integer, Integer)()

                If itemsToken IsNot Nothing AndAlso itemsToken.Type = JTokenType.Array Then
                    For Each token As JToken In DirectCast(itemsToken, JArray)
                        If token Is Nothing OrElse token.Type <> JTokenType.Object Then
                            Continue For
                        End If

                        Dim itemObj As JObject = DirectCast(token, JObject)

                        Dim productoId As Integer = ObtenerEntero(itemObj, "productoId", "ProductoId", "id", "Id")
                        Dim cantidad As Integer = ObtenerEntero(itemObj, "cantidad", "Cantidad")

                        If productoId <= 0 OrElse cantidad <= 0 Then
                            Continue For
                        End If

                        If cantidadesPorProducto.ContainsKey(productoId) Then
                            cantidadesPorProducto(productoId) += cantidad
                        Else
                            cantidadesPorProducto.Add(productoId, cantidad)
                        End If
                    Next
                End If

                If cantidadesPorProducto.Count = 0 Then
                    Return Json(New With {
                .ok = True,
                .success = True,
                .message = "No habia productos invitados para fusionar.",
                .data = New With {
                    .Fusionados = 0,
                    .Productos = 0
                }
                ...
                If carrito Is Nothing OrElse carrito.CarritoId <= 0 Then
                    Return JsonError("No se pudo crear u obtener el carrito activo del cliente.", 500)
                End If
                ...
                If detalles Is Nothing Then
                    detalles = New List(Of CarritoDetalle)()
                End If

                Dim totalUnidadesFusionadas As Integer = 0
                Dim totalProductosFusionados As Integer = 0

                For Each par As KeyValuePair(Of Integer, Integer) In cantidadesPorProducto
                    Dim productoId As Integer = par.Key
                    Dim cantidad As Integer = par.Value

                    Dim producto As Producto = Nothing

                    Try
                        producto = _productoServicio.ObtenerPorId(productoId)
                    Catch
                        producto = Nothing
                    End Try
                    ...
                    Dim existente As CarritoDetalle = detalles.FirstOrDefault(
                Function(det) det IsNot Nothing AndAlso
                              det.ProductoId = productoId AndAlso
                              String.Equals(If(det.Estado, String.Empty).Trim(), "ACTIVO", StringComparison.OrdinalIgnoreCase)
            )
                    ...
                    .Estado = "ACTIVO"
                        ...
                        Dim idGenerado As Integer = _carritoDetalleServicio.Insertar(nuevo)

                        If idGenerado > 0 Then
                            nuevo.CarritoDetId = idGenerado
                        End If

                        detalles.Add(nuevo)
                    End If

                    totalProductosFusionados += 1
                    totalUnidadesFusionadas += cantidad
                Next
            ...
            .ok = True,
            .success = True,
            .message = "Carrito invitado fusionado correctamente.",
            .data = New With {
                .CarritoId = carrito.CarritoId,
                .Fusionados = totalUnidadesFusionadas,
                .Productos = totalProductosFusionados
            }
        })

            Catch ex As Exception
                Return JsonError("No se pudo fusionar el carrito invitado: " & LimpiarMensaje(ex.Message), 500)
            End Try
        End Function
