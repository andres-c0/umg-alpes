...
Public Class Historial_Cambio_ContrasenaController
    Inherits Controller

    Private ReadOnly _servicio As Historial_Cambio_ContrasenaServicio

    Public Sub New()
        _servicio = New Historial_Cambio_ContrasenaServicio()
    End Sub
    ...
    Public Function Obtener(ByVal id As Integer) As ActionResult
        Try
            Dim entidad As Historial_Cambio_Contrasena = _servicio.ObtenerPorId(id)
    ...
    Public Function Insertar() As ActionResult
        Try
            ...
            Dim entidad As Historial_Cambio_Contrasena = JsonConvert.DeserializeObject(Of Historial_Cambio_Contrasena)(jsonBody)
    ...
    Public Function Actualizar() As ActionResult
        Try
            ...
            If entidad.HcId <= 0 Then
                Throw New Exception("Debe enviar un HcId válido para actualizar.")
            End If
    ...
    Public Function Eliminar() As ActionResult
        Try
            Dim hcId As Integer = ObtenerHcIdDesdeRequest()

            If hcId <= 0 Then
                Throw New Exception("Debe enviar un HcId válido para eliminar.")
            End If

            _servicio.Eliminar(hcId)
    ...
    Private Function ObtenerHcIdDesdeRequest() As Integer
        Dim id As Integer = 0

        If Integer.TryParse(Convert.ToString(Request("HcId")), id) AndAlso id > 0 Then
            Return id
        End If

        If Integer.TryParse(Convert.ToString(Request("hc_id")), id) AndAlso id > 0 Then
            Return id
        End If
    ...
    Private Function LimpiarMensajeError(ByVal mensaje As String) As String
        ...
        If mensaje.Contains("No existe registro relacionado en Usuario para usu_id") Then
            Return "No existe un registro relacionado en Usuario para UsuId."
        End If

        If mensaje.Contains("Historial_Cambio_Contrasena") Then
            If mensaje.Contains("actualizar") Then
                Return "No se encontró el registro a actualizar en Historial_Cambio_Contrasena."
            End If

            If mensaje.Contains("eliminar") Then
                Return "No se encontró el registro a eliminar en Historial_Cambio_Contrasena."
            End If
        End If
