...
Public Class Movimiento_InventarioController
    ...
    Private ReadOnly _servicio As Movimiento_InventarioServicio

    Public Sub New()
        _servicio = New Movimiento_InventarioServicio()
    End Sub

    <HttpGet>
    Public Function Index() As ActionResult
        Try
            Dim lista As List(Of Movimiento_Inventario) = _servicio.Listar()
    ...
    Public Function Obtener(ByVal id As Integer) As ActionResult
        Try
            Dim entidad As Movimiento_Inventario = _servicio.ObtenerPorId(id)
    ...
    Public Function Buscar(ByVal criterio As String, ByVal valor As String) As ActionResult
        Try
            Dim lista As List(Of Movimiento_Inventario) = _servicio.Buscar(criterio, valor)
    ...
    Public Function Insertar() As ActionResult
        Try
            ...
            Dim entidad As Movimiento_Inventario = JsonConvert.DeserializeObject(Of Movimiento_Inventario)(jsonBody)
    ...
    Public Function Actualizar() As ActionResult
        Try
            ...
            If entidad.MovInvId <= 0 Then
                Throw New Exception("Debe enviar un MovInvId válido para actualizar.")
            End If
    ...
    Public Function Eliminar() As ActionResult
        Try
            Dim movInvId As Integer = ObtenerMovInvIdDesdeRequest()

            If movInvId <= 0 Then
                Throw New Exception("Debe enviar un MovInvId válido para eliminar.")
            End If

            _servicio.Eliminar(movInvId)
    ...
    Private Function ObtenerMovInvIdDesdeRequest() As Integer
        Dim id As Integer = 0

        If Integer.TryParse(Convert.ToString(Request("MovInvId")), id) AndAlso id > 0 Then
            Return id
        End If

        If Integer.TryParse(Convert.ToString(Request("mov_inv_id")), id) AndAlso id > 0 Then
            Return id
        End If
        ...
        Try
            ...
            Dim token As JToken = obj("MovInvId")

            If token Is Nothing Then
                token = obj("mov_inv_id")
            End If
