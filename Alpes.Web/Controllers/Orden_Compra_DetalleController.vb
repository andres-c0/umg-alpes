...
Public Class Orden_Compra_DetalleController
    Inherits Controller

    Private ReadOnly _servicio As Orden_Compra_DetalleServicio

    Public Sub New()
        _servicio = New Orden_Compra_DetalleServicio()
    End Sub
    ...
    Public Function Obtener(ByVal id As Integer) As ActionResult
        Try
            Dim entidad As Orden_Compra_Detalle = _servicio.ObtenerPorId(id)
    ...
    Public Function Insertar() As ActionResult
        Try
            ...
            Dim entidad As Orden_Compra_Detalle = JsonConvert.DeserializeObject(Of Orden_Compra_Detalle)(jsonBody)
    ...
    Public Function Actualizar() As ActionResult
        Try
            ...
            If entidad.OrdenCompraDetId <= 0 Then
                Throw New Exception("Debe enviar un OrdenCompraDetId válido para actualizar.")
            End If
    ...
    Public Function Eliminar() As ActionResult
        Try
            Dim ordenCompraDetId As Integer = ObtenerOrdenCompraDetIdDesdeRequest()

            If ordenCompraDetId <= 0 Then
                Throw New Exception("Debe enviar un OrdenCompraDetId válido para eliminar.")
            End If

            _servicio.Eliminar(ordenCompraDetId)
    ...
    Private Function ObtenerOrdenCompraDetIdDesdeRequest() As Integer
        Dim id As Integer = 0

        If Integer.TryParse(Convert.ToString(Request("OrdenCompraDetId")), id) AndAlso id > 0 Then
            Return id
        End If

        If Integer.TryParse(Convert.ToString(Request("orden_compra_det_id")), id) AndAlso id > 0 Then
            Return id
        End If
        ...
        Try
            ...
            Dim token As JToken = obj("OrdenCompraDetId")

            If token Is Nothing Then
                token = obj("orden_compra_det_id")
            End If
