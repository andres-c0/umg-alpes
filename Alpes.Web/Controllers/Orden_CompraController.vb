...
Public Class Orden_CompraController
    ...
    Private ReadOnly _servicio As Orden_CompraServicio

    Public Sub New()
        _servicio = New Orden_CompraServicio()
    End Sub
    ...
    Public Function Obtener(ByVal id As Integer) As ActionResult
        Try
            Dim entidad As Orden_Compra = _servicio.ObtenerPorId(id)
    ...
    Public Function Insertar() As ActionResult
        Try
            ...
            Dim entidad As Orden_Compra = JsonConvert.DeserializeObject(Of Orden_Compra)(jsonBody)
    ...
    Public Function Actualizar() As ActionResult
        Try
            ...
            If entidad.OrdenCompraId <= 0 Then
                Throw New Exception("Debe enviar un OrdenCompraId válido para actualizar.")
            End If
    ...
    Public Function Eliminar() As ActionResult
        Try
            Dim ordenCompraId As Integer = ObtenerOrdenCompraIdDesdeRequest()

            If ordenCompraId <= 0 Then
                Throw New Exception("Debe enviar un OrdenCompraId válido para eliminar.")
            End If

            _servicio.Eliminar(ordenCompraId)
    ...
    Private Function ObtenerOrdenCompraIdDesdeRequest() As Integer
        Dim id As Integer = 0

        If Integer.TryParse(Convert.ToString(Request("OrdenCompraId")), id) AndAlso id > 0 Then
            Return id
        End If

        If Integer.TryParse(Convert.ToString(Request("orden_compra_id")), id) AndAlso id > 0 Then
            Return id
        End If
        ...
        Try
            ...
            Dim token As JToken = obj("OrdenCompraId")

            If token Is Nothing Then
                token = obj("orden_compra_id")
            End If
    ...
    Private Function LimpiarMensajeError(ByVal mensaje As String) As String
        ...
        If mensaje.Contains("No se puede eliminar la orden de compra porque ya tiene detalle registrado") Then
            Return "No se puede eliminar la orden de compra porque ya tiene detalle registrado."
        End If
